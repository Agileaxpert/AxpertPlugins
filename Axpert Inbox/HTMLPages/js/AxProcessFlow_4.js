...
​
    const advancedFilters = ["approved", "returned", "rejected", "notifications"];
​
    let dsName = "ds_axpertinbox";
​
    let sqlParams = {
        uname: parent.mainUserName,
filter: (this.filterkey !== undefined && this.filterkey !== null && this.filterkey !== "")
        ? this.filterkey
        : "ALL",        searchtext: this.searchtext || ""
    };
​
    // =========================
    // 🔥 TEAM
...
​
            document.querySelector('#horizontal-processbar')
...
​
        ...
        //     //ShowDimmer(false);
        // };
    openProcessTask(taskName, taskType, taskid) {debugger
​
            ShowDimmer(true);
​
​
        ...
        // }
            ...
            <iframe id="process_iframe"
                src="${url}"
                style="width:100%;height:1000%;border:0;display:block">
            </iframe>
...
const timeframe = axProcessObj.getTimeframe(dateVal.split(" ")[0] || "");                        if (!acc[timeframe]) {
                            acc[timeframe] = [];
                        }
...
const sectionIdSuffix = safeTimeframe
    .replace(/\s+/g, '')
    .replace(/[^\w-]/g, '');
                        const existingAccordion = document.getElementById(`section-${sectionIdSuffix}`);
...
​
​
            const currentSection = document.getElementById(`section-${sectionIdSuffix}`);
...
​
        ...
        const statusText = buildActionText(latest.action, latest.From, latest.to_user, latest.datetime);
​
            ...
            <div class="accordion-item">
                <h2 class="accordion-header">
                    <button class="accordion-button ${index === 0 ? '' : 'collapsed'}"
                        ...
                        <div>
                            <strong>${formatDate(row.datetime)}</strong><br>
...
​
        const links = files.split(',').map(file => {
        ...
        return `
            <div class="mt-4 action-buttons p-1">
                ${codes.map(code => {
                    ...
                    if (!btn) return "";
​
                    const url = `../../aspx/tstruct.aspx?transid=${TRANS_MAP[btn.label]}&taskid=${taskid}&act=open`;
​
                    return `
                        <button 
...
​
        // 1️⃣ FILTER BUTTONS
        const filterBtn = e.target.closest("#alllist, #activelist, #completedlist, #sentlist, #pendinglist,#approvedlist,#rejectedlist,#returnedlist,#approveBtn,#messagelist,#notificationlist");
        if (filterBtn) {
...
​
        document.querySelectorAll("#alllist, #activelist, #approveBtn, #completedlist, #sentlist, #pendinglist,#approvedlist,#rejectedlist,#returnedlist,#messagelist,#notificationlist")
...
​
        window._plannerContext = {
            source: "inbox",
            username: parent?.mainUserName || parent?.loggedUserName || parent?.userName || "",
            date: new Date()
        };
​
